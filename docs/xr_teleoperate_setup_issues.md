...
본 문서는 [scripts/](../scripts/) 폴더에 들어 있는 자동화 스크립트(install.sh, gen_certs.sh, test_pose_only.py, diagnose.sh)가 **왜 필요한가**를 설명하는 reference입니다. 각 항목 끝에 관련 커밋 해시를 표시.
...
**변경 위치**: [README.md](../README.md) Step A
...
**최종 변경 위치**: [scripts/environment.yml](../scripts/environment.yml)
...
**변경 위치**: [scripts/install.sh](../scripts/install.sh) §0
...
**변경 위치**: [scripts/install.sh](../scripts/install.sh) §5

**변경 내용**: `INSTALL_DEX_RETARGETING=1` opt-in 플래그. 기본값은 skip. Week 5(DG-5F retargeting 작업) 진입 시에만:
```bash
INSTALL_DEX_RETARGETING=1 bash scripts/install.sh
```
...
**변경 위치**: [scripts/install.sh](../scripts/install.sh) §4
...
**원인**: televuer `televuer.py:91`에서 `Vuer(host='0.0.0.0', cert=cert_file, key=key_file, ...)`를 무조건 호출. 우리는 cert 파일을 안 만들었음. 공식 README §2.2~2.3에 cert 생성 절차가 있는데 우리 scripts/README에는 누락.

**변경 위치**: [scripts/gen_certs.sh](../scripts/gen_certs.sh) (신규), [scripts/install.sh](../scripts/install.sh) §8
...
**변경 위치**: [scripts/test_pose_only.py](../scripts/test_pose_only.py) (신규)
...
**증상**: `python3 scripts/test_pose_only.py --help` 출력에 우리가 추가한 `--measure`, `--report` 옵션이 안 보이고 vuer 내부 옵션만 나옴.

**원인**: televuer 임포트 시 vuer가 자동으로 params_proto로 sys.argv를 처리. 우리 argparse가 동작할 때는 이미 argv가 가공됨.

**변경 위치**: [scripts/test_pose_only.py](../scripts/test_pose_only.py) 상단
...
**1차 변경 위치**: [scripts/gen_certs.sh](../scripts/gen_certs.sh)
...
추가로 [scripts/diagnose.sh](../scripts/diagnose.sh) 신규 — connectivity 5단계 자동 점검(cert SAN / 8012 LISTEN / curl HTTP/HTTPS / adb).
...
**변경 위치**: [scripts/test_pose_only.py](../scripts/test_pose_only.py) `--http` 옵션
...
[scripts/diagnose.sh](../scripts/diagnose.sh) §3도 HTTP/HTTPS 둘 다 시도해 서버 모드를 자동 감지하도록 수정.

[README.md](../README.md) Step G default도 `--http` 권장으로 표기.

**커밋**: `ba4641a`

---

## 2. 변경 결과: scripts/ 폴더 구조
...
conda activate tv
bash scripts/install.sh                       # clone + deps + cert
adb reverse tcp:8012 tcp:8012
python3 scripts/test_pose_only.py --http      # ← --http 권장
# 다른 터미널: bash scripts/diagnose.sh        # 4/4 OK 확인
```
...
|---|---|---|
| ADB 호환성(§1.1) | 전혀 없음 (OS-level) | scripts/README Step A 영구 유지 |
...
- [README.md](../README.md) — 다른 PC 재현 step-by-step 가이드
