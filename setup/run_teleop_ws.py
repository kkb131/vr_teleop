...
자체 ws bridge (setup/bridge_pose_store.py + setup/webxr_to_pose.html) 가 검증됨.

이 wrapper 는 그 ws bridge 를 teleop_hand_and_arm.py 에 inject:
- televuer.televuer.TeleVuer 클래스를 setup/bridge_pose_store.BridgePoseStore 로
  monkey-patch
...
Usage:
  ...
  python3 setup/run_teleop_ws.py --ee dex3 --sim
...
def _inject_bridge_pose_store() -> None:
    ...
    """
    # setup/ 디렉토리를 sys.path 에 추가 (bridge_pose_store import 위해)
...
def _resolve_teleop_path() -> Path:
    ...
    if not teleop.exists():
        print(f"[run_teleop_ws] ERROR: {teleop} 없음 — bash setup/install.sh 먼저", file=sys.stderr)
        sys.exit(1)
    return teleop
...
def _sanity_check() -> None:
    ...
    if env != "tv":
        ...
        print("                conda activate tv  → source setup/dds_env.sh  → 재시도")
        sys.exit(2)
    try:
...
def main() -> int:
    ...
    if os.environ.get("ROS_DOMAIN_ID") != "1":
        print("[run_teleop_ws] WARN: ROS_DOMAIN_ID != 1. 'source setup/dds_env.sh' 권장", flush=True)
